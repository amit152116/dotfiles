...
local function get_cached(command, cache_timeout)
  ...
  if
    cache.data[command]
    and os.difftime(os.time(), cache.timestamps[command])
      < cache.cache_time[command]
  then
...
local function get_ros2_data(command, callback, opts)
  ...
  utils.run_command(
    ...
    vim.schedule_wrap(function(_, data)
      ...
      if not opts.no_cache and cache_timeout > 0 then
        set_cache(command, results)
      end
...
local function create_ros_previewer(command, opts)
  ...
  return previewers.new_buffer_previewer {
    ...
    teardown = function(self)
      ...
      if
        previewer.timer
        and previewer.timer:is_closing(previewer.timer) == false
      then
        previewer.timer:stop()
        previewer.timer:close()
        previewer.timer = nil
          ...
          attach_mappings = function(prompt_buf)
            actions.select_default:replace(function()
              ...
              terminal.create_terminal(
                "ros2 run " .. pkg_selected.value .. " " .. node_selected.value
              )
            end)
            return true
          end,
...
local function launch_file_args(pkg, launch_file)
  ...
  local args_output =
    vim.fn.systemlist("ros2 launch --show-args " .. pkg .. " " .. launch_file)
  local arguments = {}
  ...
  for idx, line in ipairs(args_output) do
    ...
    if arg_name then
      ...
      if current_arg and arg_name_idx + 1 == idx then
        ...
        if default_value and current_arg then
          arguments[current_arg].value = default_value
        end
      end
    end
  end
